...
function generateThumbnailFromUrl(video, icon) {
    ...
    return requests_1.downloadImage(icon.url, thumbnailPath, initializers_1.THUMBNAILS_SIZE);
}
...
function getOrCreateVideoAndAccountAndChannel(options) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const videoUrl = activitypub_1.getAPUrl(options.videoObject);
        ...
        if (videoFromDatabase) {
            ...
            if (syncParam.refreshVideo === true)
                ...
                yield job_queue_1.JobQueue.Instance.createJob({ type: 'activitypub-refresher', payload: { type: 'video', videoUrl: videoFromDatabase.url } });
...
function updateVideoFromAP(options) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            yield generateThumbnailFromUrl(options.video, options.videoObject.icon);
        }
        catch (err) {
            logger_1.logger.warn('Cannot generate thumbnail of %s.', options.videoObject.id, { err });
        }
    });
}
