...
function processActivities(activities, options = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const activity of activities) {
            ...
            const actorUrl = activitypub_1.getAPUrl(activity.actor);
            if (options.signatureActor && actorUrl !== options.signatureActor.url) {
                logger_1.logger.warn('Signature mismatch between %s and %s, skipping.', actorUrl, options.signatureActor.url);
                continue;
            }
            if (options.outboxUrl && activitypub_1.checkUrlsSameHost(options.outboxUrl, actorUrl) !== true) {
                logger_1.logger.warn('Host mismatch between outbox URL %s and actor URL %s, skipping.', options.outboxUrl, actorUrl);
                continue;
            }
            const byActor = options.signatureActor || actorsCache[actorUrl] || (yield actor_1.getOrCreateActorAndServerAndModel(actorUrl));
            ...
            try {
                yield activityProcessor(activity, byActor, options.inboxActor);
            }
            catch (err) {
