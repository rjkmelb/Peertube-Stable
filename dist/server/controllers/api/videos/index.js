...
videosRouter.get('/:id', middlewares_1.optionalAuthenticate, middlewares_1.asyncMiddleware(middlewares_1.videosGetValidator), middlewares_1.asyncMiddleware(middlewares_1.checkVideoFollowConstraints), getVideo);
...
function getVideo(req, res) {
    ...
    if (videoInstance.isOutdated()) {
        job_queue_1.JobQueue.Instance.createJob({ type: 'activitypub-refresher', payload: { type: 'video', videoUrl: videoInstance.url } })
            .catch(err => logger_1.logger.error('Cannot create AP refresher job for video %s.', videoInstance.url, { err }));
    }
